...
import {isValidMathEquation} from '@/domain/factories/CardFactory';
...
export function useCardSelection(isMyTurn: boolean): UseCardSelectionResult {
  ...
  const possibleActions = useMemo(() => {
    ...
    if (hasNumbers >= 3) {
      // Check if the selected number cards form a valid math equation
      if (isValidMathEquation(numberCards)) {
